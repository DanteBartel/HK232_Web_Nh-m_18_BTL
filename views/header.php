			...
			<nav class="flex items-center py-3">
				...
				<div class="basis-7/12 lg:basis-6/12 flex flex-wrap justify-end">
					...
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
						class="xl:h-10 xl:w-20 px-1 h-10 w-10 border-r border-violet-300 cursor-pointer hover-fade">
					...
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"
						class="xl:h-10 xl:w-20 px-1 h-10 w-10 cursor-pointer hover-fade">
						<a
							href="index.php?action=accounts<?php echo (isset($_SESSION['user_type']) && $_SESSION['user_type'] == 1) ? '&verb=edit&id=' . $_SESSION['account_id'] : '' ?>">
							<path fill="#8a79be"
					...
					<div class="relative md:hidden">
						...
						<div id="dropdownMenu"
							class="absolute right-0 mt-2 border-2 border-gray-500 rounded-md shadow-lg hidden bg-purple-100">
							<div class="py-1">
								<div class="w-40 uppercase font-bold text-center"><a href="index.php?action=books&page=1">To Books</a>
								</div>
								<div
									class="w-40  px-1 uppercase font-bold text-center <?php echo isset($_SESSION['loggedin']) ? " hidden" : "" ?>">
									<a href="index.php?action=login">To Login</a>
								</div>
								<div class="w-40 px-1 pb-2 uppercase font-bold text-center">
									<?php echo "Hello " . (isset($_SESSION['username']) ? $_SESSION['username'] : "Guest"); ?>
								</div>
								<div class="w-24 pl-12 pb-2 <?php echo isset($_SESSION['loggedin']) ? "" : " hidden" ?>">
									<a href="index.php?action=logout"
										class="bg-rose-700 hover:bg-rose-500 p-1 border-solid border-2 border-black text-white font-bold rounded">Logout</a>
								</div>
							</div>
						</div>
					</div>
					...
					<div id="flexMenu" class="flex justify-end">
						<!--Books  -->
						<div
							class="w-20 h-10 py-1 border-2 border-violet-200 hover:bg-violet-900 rounded-md bg-violet-900 bg-opacity-70 text-white font-bold text-center">
							<a href="index.php?action=books&page=1">To Books</a>
						</div>

						<!-- Login -->
						<div
							class="w-20 h-10 py-1 border-2 border-violet-200 hover:bg-violet-900 rounded-md bg-violet-900 bg-opacity-70 text-white font-bold text-center <?php echo isset($_SESSION['loggedin']) ? " hidden" : "" ?>">
							<a href="index.php?action=login">To Login</a>
						</div>

						<!-- User account display -->
						<div class="w-20 pl-4 font-bold">
							<?php
							echo "Hello, " . (isset($_SESSION['username']) ? $_SESSION['username'] : "Guest");
							?>
						</div>
